...
class ToolsViewModel : ViewModel() {
    ...
    fun loadStats(ctx: Context) = viewModelScope.launch(Dispatchers.IO) {
        ...
        val patchedDir = File(android.os.Environment.getExternalStorageDirectory(), "AzlukPatcher/Patched")
    ...
    fun scanWifi(ctx: Context) = viewModelScope.launch(Dispatchers.IO) {
        _state.update { it.copy(wifiScanning = true, wifiNets = emptyList()) }
        try {
            val wm = ctx.applicationContext.getSystemService(Context.WIFI_SERVICE) as WifiManager
            val enabled = wm.isWifiEnabled
            _state.update { it.copy(wifiEnabled = enabled) }
            if (!enabled) { _state.update { it.copy(wifiScanning = false) }; return@launch }

            wm.startScan()
            delay(2000) // wait for scan

            val results = wm.scanResults
            val nets = results.map { r ->
                val ssid = if (Build.VERSION.SDK_INT >= 33) r.wifiSsid?.toString()?.trim('"') ?: ""
                           else @Suppress("DEPRECATION") r.SSID ?: ""
                val sec = when {
                ...
                WifiNet(ssid.ifEmpty { "<hidden>" }, r.BSSID ?: "", r.level, r.frequency, sec, ch)
            }.sortedByDescending { it.rssi }
            _state.update { it.copy(wifiNets = nets, wifiScanning = false) }
        } catch (e: Exception) {
            _state.update { it.copy(wifiScanning = false, message = "Scan failed: ${e.message}") }
        }
    }
    ...
    Column(
        ...
        Box(
            ...
            Row(verticalAlignment = Alignment.CenterVertically) {
                ...
                Text("Tools", color = AzlukOnBg, fontSize = 20.sp, fontWeight = FontWeight.Bold)
            }
        }
...
private fun DeviceTab(state: ToolsState, ctx: Context, vm: ToolsViewModel) {
    ...
    val ramPct  = if (state.ramTotal > 0) (ramUsed.toFloat() / state.ramTotal * 100).toInt() else 0
        ...
        item {
            GlassCard {
                Column(Modifier.padding(16.dp)) {
                    ...
                    val extTotal = android.os.Environment.getExternalStorageDirectory().totalSpace
                    val extFree  = android.os.Environment.getExternalStorageDirectory().freeSpace
                    StorageRow("Storage Free", extFree)
                    StorageRow("Storage Total", extTotal)
                }
            }
        }
        // Quick links
        item {
            Text("Quick Access", color = AzlukOnSurface, fontSize = 12.sp, fontWeight = FontWeight.SemiBold,
        ...
        item {
            Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                ...
                QuickLink("Private DNS", Icons.Default.Dns) {
                    ...
                    catch (e: Exception) { ctx.startActivity(Intent(Settings.ACTION_WIRELESS_SETTINGS)) }
                }
                QuickLink("AzlukPatcher Folder", Icons.Default.Folder) {
                    val dir = File(android.os.Environment.getExternalStorageDirectory(), "AzlukPatcher")
                    dir.mkdirs()
...
@Composable
private fun WifiTab(state: ToolsState, ctx: Context, vm: ToolsViewModel) {
        ...
        GlassCard {
            Column(Modifier.padding(16.dp)) {
                ...
                listOf(
                    ...
                    Row(
                        Modifier
                            ...
                            .clickable {
                                ...
                                android.widget.Toast.makeText(ctx, "Set hostname: $host", android.widget.Toast.LENGTH_LONG).show()
                            }
                    ...
                    Divider(color = AzlukSurface2, thickness = 0.5.dp)
                }
            }
        }
...
@Composable
private fun QuickLink(label: String, icon: androidx.compose.ui.graphics.vector.ImageVector, onClick: () -> Unit) {
    Card(
        ...
        Row(
            ...
            Spacer(Modifier.width(12.dp))
            Text(label, color = AzlukOnBg, fontSize = 14.sp, Modifier.weight(1f))
